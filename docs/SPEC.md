...
| `multidiff.snapshot.openMode` | `"virtual" \| "tempFile"` | `"virtual"` | スナップショットの開き方。`virtual` は `multidiff:` URI、`tempFile` は一時ファイルミラー |
...
│   ├── historicalLanguageFeatureService.ts -- 履歴リビジョン言語機能
...
│   ├── scrollAligned.ts                   -- Ctrl+Up/Down アライン済みスクロール
...
│   │   └── prototypeComparePanel.ts       -- プロトタイプパネル (参考保持)
│   ├── decorations/
...
├── compatibility/
│   ├── definitionBridgeProvider.ts        -- 定義ジャンプブリッジ
│   ├── hoverBridgeProvider.ts             -- ホバーブリッジ
│   └── referenceBridgeProvider.ts         -- 参照ブリッジ
├── configuration/
│   └── extensionConfiguration.ts          -- 型安全設定アクセサ
└── util/
    ├── disposable.ts                      -- Disposable ユーティリティ
    ├── hash.ts                            -- ハッシュユーティリティ
    ├── output.ts                          -- 出力ログ
    └── process.ts                         -- 子プロセス実行ユーティリティ
```
