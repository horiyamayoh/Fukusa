# MultiDiffViewer 設計書（改訂版ドラフト v0.2 / Native Editor First）

- 作成日: 2026-03-11
- 対象: VS Code 拡張機能 **MultiDiffViewer**
- この版の位置づけ:
...
このため、MultiDiffViewer 側が「native editor で開く」だけでは十分ではありません。  
...
2. MultiDiffViewer 独自の DefinitionProvider が受ける
...
### 改訂後の一言での方針
**「MultiDiffViewer は、独自描画の diff viewer ではなく、VS Code 標準 editor / diff editor を束ねる履歴閲覧オーケストレータとして作る」**
